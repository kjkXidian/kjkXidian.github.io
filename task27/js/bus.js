...
function BUS(command, orbit, /*optional*/speed, /*optional*/consumeRate, /*optional*/chargeRate){
	//console.log(orbit + " " + speed + " " + consumeRate + " " + chargeRate);
	var order ={id: orbit, command: command};
	var sendMessage = function(order) { //发送命令
		if(order.command === 'createShip' && speed != undefined && consumeRate != undefined && chargeRate != undefined){
			console.log("ready to create");
			var ship = createShip(order.id, speed, consumeRate, chargeRate);
			nowShip.push(ship);
		}else{
			if(nowShip.length != 0){
	...
	var Msg=[];
	if(Math.ceil(Math.random()*10) > 1){//模拟 10% 的丢包率
		sendMessage(order);
	...
	if(msgLen.length >=9 ){//控制提示窗口的信息数量不超过10条
